...
public class Order {
    ...
    @JoinColumn(name = "customer_id",
            ...
            foreignKey = @ForeignKey(name = "order_customer_fk1"))
    private Customer customerOrder;

    @ManyToOne(fetch = FetchType.LAZY)
    @JoinColumn(name = "ward_id",
            referencedColumnName = "ward_id",
            foreignKey = @ForeignKey(name = "order_ward_fk2"))
    private Ward wardOrder;

    @ManyToOne(fetch = FetchType.LAZY)
    @JoinColumn(name = "employee_id",
            referencedColumnName = "employee_id",
            foreignKey = @ForeignKey(name = "order_employee_fk3"))
    private Employee employeeOrder;

    @ManyToOne(fetch = FetchType.LAZY)
    @JoinColumn(name = "order_status_id",
                referencedColumnName = "order_status_id",
                foreignKey = @ForeignKey(name = "order_orderStatus_fk4"))
