...
const OrderRight = ({ reRenderData, setReRenderData }) => {
    // Thứ ngày tháng
    let today = new Date();
    ...
    useEffect(() => {
        const getOrderQuantity = async () => {
            try {
                const orderQuantityRes = await axios.get("http://localhost:8080/order/get-order-quantity");
                setOrderQuantity(orderQuantityRes.data.orderQuantity);
            } catch (err) {
                console.log("Lỗi: ", err);
            }
        }
    ...
    console.log("Số lượng đơn hàng: ", orderQuantity);
            ...
            <SalesAnalytics>
                ...
                <Item className="online">
                    ...
                    <ItemRight>
                        ...
                        <h3 className="success" style={{ fontSize: "1.2rem" }}>{orderQuantity}</h3>
                    </ItemRight>
                </Item>
