...
const CategoryRight = ({ reRenderData, setReRenderData }) => {
    // Thứ ngày tháng
    let today = new Date();
    ...
    useEffect(() => {
        const getcategoryQuantity = async () => {
            try {
                const categoryQuantityRes = await axios.get("http://localhost:8080/category/get-category-quantity");
                setCategoryQuantity(categoryQuantityRes.data.categoryQuantity);
            } catch (err) {
                console.log("Lỗi: ", err);
            }
        }
    ...
    console.log("Số lượng danh mục: ", categoryQuantity);
            ...
            <SalesAnalytics>
                ...
                <Item className="online">
                    ...
                    <ItemRight>
                        ...
                        <h3 className="success" style={{ fontSize: "1.2rem" }}>{categoryQuantity}</h3>
                    </ItemRight>
                </Item>
                <Item className="add-product"
                    onClick={() => openModal({ type: "addCategory" })}
                >
                    <Add />
                    <h3>Thêm danh mục</h3>
                </Item>
