...
const CustomerMain = ({ reRenderData, setReRenderData }) => {
    ...
    useEffect(() => {
        const findCustomer = async () => {
            try {
                const findCustomerRes = await axios.get(`http://localhost:8080/customer/get-customer-by-customer-name/${search}`);
                setCustomer(findCustomerRes.data);
                console.log("Kết quả tìm Khách hàng trong effect: ", findCustomerRes.data);
            } catch (err) {
    ...
    const customerChangePage = customer
            ...
            return (
                <Tr>
                    ...
                    <Td style={{ display: 'flex', justifyContent: 'center', alignItems: 'center' }}>
                        <ImgDanhMuc src={customerItem.customerAvatar} />
                    </Td>
                    <Td>{customerItem.customerName}</Td>
                    <Td>{customerItem.customerGender}</Td>
                    <Td>{customerItem.customerEmail}</Td>
                    <Td>{customerItem.customerPhone}</Td>
                    <Td className="info">
                        <ButtonInfo
                            onClick={() => openModal({ type: "detailCustomer", customer: customerItem })}
                        >
                    ...
                    <Td className="primary">
                        <ButtonDelete
                            onClick={() => openModal({ type: "deleteCustomer", customer: customerItem })}
                        >
                            <DeleteSweepOutlined />
                        </ButtonDelete>
                    </Td>
    ...
    return (
        <Container>
            ...
            <SearchWrapper className={isSearch ? "active" : null}>
                <InputHolder>
                    <Input ref={InputRef} type="text" placeHolder="Nhập vào Tên khách hàng" onChange={(e) => setSearch(e.target.value)} />
                    ...
                    <Tbody>
                        {
                            customer !== null
                                ...
                                customerChangePage
                                :
                                (customer.slice(0, 12).map(customerItem => (
                                    <Tr>
                                        <Td>{customerItem.customerId}</Td>
                                        <Td style={{ display: 'flex', justifyContent: 'center', alignItems: 'center' }}>
                                            <ImgDanhMuc src={customerItem.customerAvatar} />
